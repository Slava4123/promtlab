...
// Ключ sessionStorage для сохранения return_url через OAuth-flow.
// OAuth redirect через Google/GitHub/Yandex теряет query-параметры, поэтому
// сохраняем локально, затем oauth-callback.tsx забирает и использует для
// финального редиректа.
const RETURN_URL_KEY = "pv:oauth_return_url"

// rememberReturnURL вызывается перед window.location.href на OAuth provider.
function rememberReturnURL(url: string | null): void {
  if (url) {
    try {
      sessionStorage.setItem(RETURN_URL_KEY, url)
    } catch {
      // sessionStorage может быть недоступен (приватный режим с disabled storage)
    }
  }
}
    ...
    <AuthLayout>
      ...
      <div className="space-y-2.5">
        <Button
          ...
          onClick={() => { rememberReturnURL(returnURL); window.location.href = withReferral("/api/auth/oauth/github") }}
        >
        ...
        <Button
          ...
          size="lg"
          className="w-full gap-2.5 border-border bg-card text-foreground hover:bg-muted"
          onClick={() => { rememberReturnURL(returnURL); window.location.href = withReferral("/api/auth/oauth/google") }}
        >
        ...
        <Button
          ...
          size="lg"
          className="w-full gap-2.5 border-border bg-card text-foreground hover:bg-muted"
          onClick={() => { rememberReturnURL(returnURL); window.location.href = withReferral("/api/auth/oauth/yandex") }}
        >
