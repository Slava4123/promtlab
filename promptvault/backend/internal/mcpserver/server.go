...
Аккаунт:
- whoami — информация о текущем пользователе (id, email, plan).

Аналитика и аудит (Phase 14):
- team_activity_feed — лента событий команды (prompt.*, collection.*, share.*, member.*, role.*).
  Cursor-пагинация через next_cursor. Требует membership в команде.
- analytics_summary — компактная личная сводка (top-3 промпты, total uses/views) за период.
  Period clamp'ится по тарифу (Free=7d, Pro=90d, Max=365d).
- analytics_team_summary — аналог для команды + top-3 contributors.
- get_prompt_versions — теперь возвращает actor (email/name) автора версии.
...
// Options — опциональные зависимости MCP-сервера. OAuth-слой и
// resourceMetadataURL — опциональны для backward-compat с уже существующим
// static-API-key сценарием: пока OAuthValidator=nil, работает только pvlt_*
// flow, как раньше.
type Options struct {
	// UserRPM — per-user rate limit. 0 = отключён.
	UserRPM int
	// OAuthValidator — реализация ValidateAccessToken (usecases/oauth_server.Service).
	// Если nil, OAuth-flow отключён.
	OAuthValidator OAuthValidator
	// ResourceMetadataURL — полный URL до /.well-known/oauth-protected-resource.
	// Нужен для WWW-Authenticate header в 401-ответах MCP-клиентам.
...
func NewMCPServer(
	...
	if opts.UserRPM > 0 {
		handler = ratelimit.ByUserID(opts.UserRPM, func(r *http.Request) uint {
			return authmw.GetUserID(r.Context())
		})(handler)
	}

	// Auth middleware wraps everything. Поддерживает и статические pvlt_*,
	// и OAuth access-токены pvoat_* (если opts.OAuthValidator задан).
	authed := APIKeyAuth(apiKeySvc, opts.OAuthValidator, opts.ResourceMetadataURL)(handler)
