// shadcn/ui Select поверх Radix UI Select — полностью кастомный dropdown с
// portal-рендерингом, keyboard navigation, ARIA и нашей темой.
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      sideOffset={4}
      className={cn(
        'relative z-50 max-h-[min(var(--radix-select-content-available-height),18rem)] min-w-[8rem] overflow-hidden rounded-md border border-(--color-border) bg-(--color-card) text-(--color-card-foreground) shadow-lg',
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn('py-1.5 pl-8 pr-2 text-[10px] font-medium uppercase tracking-wide text-(--color-muted-foreground)', className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      'data-[state=checked]:bg-(--color-primary)/10 data-[state=checked]:text-(--color-primary)',
