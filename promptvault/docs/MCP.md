...
**Версия сервера:** `v1.2.0` (см. `server.json`). В v1.2 добавлены 5 tools: `list_teams`, `whoami`, `list_trash`, `restore_prompt`, `purge_prompt`; квотирование покрыто 13 из 30 tools.
...
| `list_trash` | Содержимое корзины (soft-deleted промпты, пагинация) | ✅ |
...
Из 30 tools дневную MCP-квоту едят только **13** write/destructive операций, перечисленные выше с ✅ в колонке «Ест квоту». Остальное (14 read-only + 3 UX-toggle: `prompt_favorite`, `prompt_pin`, `prompt_increment_usage`) — бесплатно.

Лимиты по тарифам: Free — 5 платных вызовов в день, Pro — 30, Max — безлимит. При превышении backend возвращает `402 Payment Required`.
